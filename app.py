...
# S[cid] = {
#   step, budget, outbox,
#   line, title, desc, media,
#   store: { line_name: { "ads": [ {"title":..., "desc":..., "media":..., "cpm":..., "msg":..., "spent":...}, ... ] } }
# }
S = {}
...
def send(cid, text, **kwargs):
    """Envía y registra message_id para posibles borrados propios."""
...
def clean_private(cid):
    """En chat privado: borra mensajes del bot (limitación Telegram) y reinicia estado (conservando store)."""
    state = st(cid)
    deleted = 0
...
def simulate_costs(budget):
    """
    Devuelve una tupla por cada anuncio: (mensajes_estimados, costo_por_mensaje, gasto_asignado)
    Reparte el presupuesto con pesos aleatorios y estima mensajes; es solo para simulación.
    """
    # pesos aleatorios para simular que Meta favorece algunos anuncios
    weights = [random.uniform(0.8, 1.2), random.uniform(0.8, 1.2), random.uniform(0.8, 1.2)]
    s = sum(weights)
    alloc = [budget * w / s for w in weights]  # gasto por anuncio
    # costo por mensaje típico simulado (COP por msg). Rango 800 - 2500 aprox.
    base_cpm = [random.uniform(800, 2500) for _ in range(3)]
    # mensajes estimados = gasto / cpm
    msgs = [max(1, int(a / c)) for a, c in zip(alloc, base_cpm)]
    # coste por mensaje final redondeado
...
def summarize(cid):
    ...
    # simulación de costos por anuncio
    sim = simulate_costs(budget)  # [(msgs, cpm, spent), ...]
    (m1, c1, s1), (m2, c2, s2), (m3, c3, s3) = sim
    ...
    send(cid,
        ...
        "_(Los valores de costo por mensaje y gasto son simulados para fines de prueba.)_"
    , parse_mode="Markdown")
    send_main_menu(cid)
...
# =====================
# Callbacks del menú (incluye eliminar línea/anuncio)
# =====================
...
def on_menu(c):
    ...
    if data == "menu_sim":
        ...
        help_txt = (
            ...
            "• Menú: Ver/Eliminar LÍNEA o ANUNCIO; el resumen incluye *Costo por mensaje* simulado.\n"
        )
...
# Ignora comandos en handler de texto para evitar duplicados
